...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["ktp-file"]) && $_FILES["ktp-file"]["error"] == 0) {
        $target_dir = "uploads/";
        $target_file = $target_dir . basename($_FILES["ktp-file"]["name"]);
        move_uploaded_file($_FILES["ktp-file"]["tmp_name"], $target_file);
        $ktp_filename = basename($_FILES["ktp-file"]["name"]);

        // Simpan nama file ke database
        $sql = "UPDATE tb_user SET ktp='$ktp_filename' WHERE id=1"; // Sesuaikan dengan id user Anda
    ...
    if (isset($_FILES["kk-file"]) && $_FILES["kk-file"]["error"] == 0) {
        $target_dir = "uploads/";
        $target_file = $target_dir . basename($_FILES["kk-file"]["name"]);
        move_uploaded_file($_FILES["kk-file"]["tmp_name"], $target_file);
        $kk_filename = basename($_FILES["kk-file"]["name"]);

        // Simpan nama file ke database
        $sql = "UPDATE tb_user SET kk_filename='$kk_filename' WHERE id=1"; // Sesuaikan dengan id user Anda
        if ($conn->query($sql) === TRUE) {
        ...
        <div class="row mb-3">
            <div class="col-md-12">
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="<?php echo"$nama" ?>">
                </div>
            </div>
        </div>
        <div class="row mb-3">
        ...
        <div class="row mb-3">
            <div class="col-md-12">
                <div class="input-group mb-3">
                    ...
                    <input type="tel" class="form-control" placeholder="<?php echo"$nomor_telepon" ?>">
                </div>
            </div>
        </div>
        <div class="row mb-1">
            <div class="col-md-6">
                ...
                <img src="assets/images/ktp.jpeg" alt="Ganti Foto KTP" style="max-width: 500px; margin-top:10px;">
            </div>
            <div class="col-md-6">
                <div class="input-group">
                    <!-- Hidden input file element for Kartu Keluarga -->
                ...
                <img src="assets/images/kk.jpeg" alt="Ganti Foto KTP" style="width: 450px; margin-top:10px;">
            </div>
        </div>
        <div class="row mb-1">
            <div class="col-md-12">
                <button class="btn btn-secondary mt-3" type="submit" style="width:650px">Simpan Perubahan</button>
            </div>
        </div>
    </form>
</div>
